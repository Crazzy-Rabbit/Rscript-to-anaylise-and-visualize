...
plot_locuszoom <- function(data, genelist, flank=500000) {
    top_snp=data$topsnp
    region_data=datapltX$region_dt 
    pltX=region_data$POS/1e6; 
    pltY=-log10(region_data$p); 
    pltcol=region_data$color 

    yMAX=ceiling(max(pltY))
    offset_gene=yMAX/4
    dev_axis=1.5
    xmin=min(pltX,na.rm=T)-0.01; 
    xmax=max(pltX,na.rm=T)+0.01; 
    ...
    plot(pltX,pltY,pch=21,bg=pltcol,col="gray30",cex=1.8,lwd=0.2,yaxt="n",bty="n",
    xlab=xlab, ylab="", xlim=c(xmin,xmax), ylim=c(ymin,ymax))
    x_ticks=round(seq(xmin,xmax,length.out=6),2)
    axis(1,at=x_ticks,lwd=1)
    axis(2,at=seq(0,yMAX,by=yMAX/4),labels=round(seq(0,yMAX,yMAX/4),0),lwd=1,las=1)
    mtext(ylab, side=2, line=3, at=(yMAX*1/2))
    segments(x0=min(pltX), y0=-0.5, x1=max(pltX), y1=-0.5, col="gray50", lwd=1, lty=2) 
    ...
    gene_chr=top_snp$CHR
    gene_start=top_snp$POS-flank; 
    gene_end=top_snp$POS+flank
    generow=glist[CHR == gene_chr & start >= gene_start & end <= gene_end, ]
    ...
    required_rows=max(generow$layer)
    axis.start=ymin/2-1
    total_lay=abs(offset_gene+dev_axis)/5
    if (required_rows %% 2==1){
        median_layer=(required_rows+1)/2
        generow[, y_pos := axis.start+(layer-median_layer)*0.8]
    } else {
        median_upper=required_rows/2
        generow[, y_pos := axis.start+(layer-median_upper-0.5)*0.8*total_lay]
    }

    for(i in 1:nrow(generow)) {
        arrow_code <- ifelse(generow$strand[i] == "+", 2, 1)
        gene_len <- generow$end[i]-generow$start[i]
        if (gene_len < 0.001) {
            arrows(x0=generow$x_center[i]-0.0005, y0=generow$y_pos[i], x1=generow$x_center[i]+0.0005, y1=generow$y_pos[i], length=0.07, code=arrow_code, col="darkgoldenrod", lwd=1.5)} 
        else {
            arrows(x0=generow$start[i], y0=generow$y_pos[i], x1=generow$end[i], y1=generow$y_pos[i], length=0.07, code=arrow_code, col="darkgoldenrod", lwd=1.5)}
        text(x=generow$x_center[i], y=generow$y_pos[i], substitute(italic(gene), list(gene=generow$GENE[i])), pos=3, offset=0.3, cex=0.8, col="black")
    }
    ...
    y_bottom=usr[4]-0.3*(usr[4]-usr[3])-down_margin  
    y_top=usr[4]-down_margin                           
    y_pos=seq(y_bottom, y_top, length.out=length(breaks)) 
    
    tit=expression(italic(r)^2)
    for (i in 1:(length(breaks)-1)) {
        rect(legend_x_left, y_pos[i], legend_x_right, y_pos[i+1], col=color_pal[i], border=NA)
    }
    text(x=legend_x_left, y=y_pos, labels=breaks, pos=2, cex=0.7, xpd=TRUE)
    text(x=legend_x_left, y=y_top, labels=tit, pos=3, cex=0.9)
}
